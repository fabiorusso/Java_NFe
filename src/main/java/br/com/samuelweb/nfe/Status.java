...
public class Status {
	...
	public static TRetConsStatServ statusServico(TConsStatServ consStatServ, boolean valida, String tipo, String cnpj)
		...
		CertificadoUtil certificadoUtil = new CertificadoUtil(cnpj);
		...
		try {
			...
			if (valida) {
				String erros = Validar.validaXml(xml, Validar.STATUS, cnpj);
			...
			if (BA && !nfce) {
				...
				NfeStatusServicoStub stubBA = new NfeStatusServicoStub(nfce
						? WebServiceUtil.getUrl(ConstantesUtil.NFCE, ConstantesUtil.SERVICOS.STATUS_SERVICO, cnpj)
						: WebServiceUtil.getUrl(ConstantesUtil.NFE, ConstantesUtil.SERVICOS.STATUS_SERVICO, cnpj));
